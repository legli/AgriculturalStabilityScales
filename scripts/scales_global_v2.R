...
#   # detrend yields
#   dfProductionWorldTime <- dfProductionFullFinal[which(dfProductionFullFinal$Year>=yearStart&dfProductionFullFinal$Year<=(yearStart+9)),]
#   noRegions <- length(unique(dfProductionWorldTime$Area))
#   
#   lsScales <- lapply(1:noRegions,function(s){
#     show(s)
#     lsRep <- lapply(1:10,function(n){
#       randomCountries <- sample(as.character(dfProductionWorldTime$Area),s)
#       vecCrop <- unique(Reduce(intersect,lapply(randomCountries,function(r){dfProductionWorldTime[dfProductionWorldTime$Area==r,"Crop"]})))
#       df <- data.frame(area=NA,stability=NA,yield=NA)
#       if (length(vecCrop)>0)
#       {
#         dfProductionRegion <- dfProductionWorldTime[which(dfProductionWorldTime$Area%in%randomCountries&dfProductionWorldTime$Crop%in%vecCrop),]
#         dfProductionRegionAgg <- aggregate(cbind(Production,AreaHarvested)~Year,dfProductionRegion,sum)
#         dfProductionRegionAgg$Yield <- dfProductionRegionAgg$Production/dfProductionRegionAgg$AreaHarvested
#         dfProductionRegionAgg$YieldDet <-resid(lm(Yield ~ Year^2,data=dfProductionRegionAgg)) 
#         area <- sum(dfProductionRegionAgg$AreaHarvested)
#         stability <- mean(dfProductionRegionAgg$Yield,na.rm=T)/sd(dfProductionRegionAgg$YieldDet,na.rm=T)
#         yield <- mean(dfProductionRegionAgg$Yield,na.rm=T)
#         df <- data.frame(area,stability,yield)
#       }
#       df
#     })
#     dfRep <- do.call(rbind,lsRep)
#     dfRep$prop <- s
#     dfRep
#   })
#   dfScales <- do.call(rbind,lsScales)
