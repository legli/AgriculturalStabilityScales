...
# regional
intersectRegionID <- intersect(mapRegion,polygonID)
intersectRegionID$area_sqkm <- area(intersectRegionID) / 1000000
writeOGR(intersectRegionID, dsn = 'spatial', layer = 'regionsClimateID', driver = "ESRI Shapefile",overwrite_layer = T)
# intersectRegionID <- readOGR("spatial/regionsClimateID.shp")

# farm
intersectFarmID <- intersect(mapFarm,polygonID)
intersectFarmID$area_sqkm <- round(area(intersectFarmID) / 1000000,31)
writeOGR(intersectFarmID, dsn = 'spatial', layer = 'farmsClimateID', driver = "ESRI Shapefile",overwrite_layer = T)
# intersectFarmID <- readOGR("spatial/farmsClimateID.shp")
...
# regional
dfClimateRegional <- functionWeigthedClimate(intersectRegionID@data,"NUTS_ID")
head(dfClimateRegional)
write.csv(dfClimateRegional,"datasetsDerived/climate_regional.csv",row.names = F)
# regional
dfClimateFarm <- functionWeigthedClimate(intersectFarmID@data,"district")
head(dfClimateFarm)
write.csv(dfClimateFarm,"datasetsDerived/climate_farm.csv",row.names = F)
