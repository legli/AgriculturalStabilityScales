...
dfClimateFinalArea$cropland1 <- ((dfClimateFinalArea$areaHA/dfClimateFinalArea$areaTot)*dfClimateFinalArea$cropland2000AD)
dfClimateFinalArea$cropland2 <- ((dfClimateFinalArea$areaHA/dfClimateFinalArea$areaTot)*dfClimateFinalArea$cropland2010AD)

# get cropland by region
sum(is.na(dfClimateFinalArea))
dfCroplandTot <- aggregate(cbind(cropland1,cropland2)~REGION_ID,dfClimateFinalArea,sum)
head(dfCroplandTot)
...
max(c(dfClimateFinalArea$weight1,dfClimateFinalArea$weight2),na.rm=T)
...
min(dfClimateFinalAreaAgg[2:41]) #  3.456387 -> check for negative values -> would be problematic for instability calculation
