...
#     if (sum(!is.na(dfCrop$Production))>=10&sum(dfCrop$Production>0,na.rm=T)>0){
...
#       if (sum(!is.na(dfCrop$Production))>=10){
...
dfProductionFull_Final <- dfProductionFull_Final[which(dfProductionFull_Final$Area%in%vecCountryFinal),]
...
save(dfProductionFull_Final, file="datasetsDerived/dfProductionFull_Final_global.RData")
...
lsAll <- lapply(vecCountryFinal,function(ctry){
  ...
  vecCrop <- unique(dfProductionFull_Final[which(dfProductionFull_Final$Area==ctry),"Group2"])
