...
dfYieldCalories <- dfYieldCalories[,c("Area","Region","Year","Production","AreaHarvested","Yield")]
...
head(dfYieldCalories)
dfProductionRegion <- aggregate(cbind(Production,AreaHarvested)~Year+Region,dfYieldCalories[which(dfYieldCalories$Area%in%vecCountryFinal),],sum)
lsProductionRegion <- lapply(unique(dfProductionRegion$Region),function(r){
  dfRegion <- dfProductionRegion[which(dfProductionRegion$Region==r),]
  dfRegion$ProductionDet <- resid(loess(Production ~ Year,data=dfRegion))
  dfRegion$Yield <- dfRegion$Production/dfRegion$AreaHarvested
  dfRegion
})
dfProductionRegion <- do.call(rbind,lsProductionRegion)
...
lsAll <- lapply(vecCountryFinal,function(ctry){
  ...
  dfProductionSumCtry <- dfProductionCaloriesFinal[which(dfProductionCaloriesFinal$Area==ctry),]
