...
funCombinePlot <- function(type,mod1,mod2,namPred,tit,color,yLength,yname){
  ...
  if (type=="farm")
  {
    ...
    vecName <- c("Diversity","sqrt(Fertilizer)","sqrt(Irigation)","Temperature instability","Area harvested") 
  }
  ...
  if (type=="farm")
  {
    fig <- fig + geom_vline(xintercept=4.5,size=0)
  }  
  fig
}
...
funFig1 <- function(dfInput,variable,map,level,b,colVec,tit)
{
